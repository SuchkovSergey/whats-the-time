...
const mapper = ([key, value]) => {
  ...
  const currentCountry = lookup.countries.find((el) => el.iso2 === code);
  const cityName = cityNamePrettier(key.split('/')[1]);
  if (!currentCountry || !code || cityName === '' || cityName.includes('+')) { return null; }
  return {
...
const reducer = (acc, element) => {
  ...
  if (!acc[continentName]) {
    acc[continentName] = { [countryName]: { code, cities: { [cityName]: newElement } } };
